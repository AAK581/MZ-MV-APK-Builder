...
param(
    ...
    [string]$ExtraKey = "C",
    [switch]$PlayMode,
    [ValidateSet("ads","support")][string]$PlayVariant = "ads",
    [string]$AdMobAppId = "",
    [string]$AdUnitId = "",
    [string]$ProductId = "",
    [string]$PriceLabel = '$0.99',
    [string]$OutputDir = ""
)
...
# ------------------------------------------------------------ build pipeline ---
# Per-store runtime config for Play-mode builds. The generated monetization.js
# reads window.MOBILE_CONFIG and stays inert when ads are off.
function New-MobileConfig([bool]$ads, [bool]$testAds, [string]$adUnit, [string]$productId, [string]$priceLabel) {
    $a = if ($ads) { "true" } else { "false" }
    $t = if ($testAds) { "true" } else { "false" }
    return "// Generated by RMMZ APK Builder - per-store configuration.`n" +
           "window.MOBILE_CONFIG = {`n" +
           "    ads: $a,`n" +
           "    testAds: $t,`n" +
           "    interstitialAdUnitId: `"$adUnit`",`n" +
           "    removeAdsProductId: `"$productId`",`n" +
           "    removeAdsPriceLabel: `"$priceLabel`"`n" +
           "};`n"
}

# Support-variant config: no ads, just the tip purchase. support.js stays
# inert when support is false (itch flavor overlay).
function New-SupportConfig([bool]$support, [string]$productId, [string]$priceLabel) {
    $s = if ($support) { "true" } else { "false" }
    return "// Generated by RMMZ APK Builder - per-store configuration.`n" +
           "window.MOBILE_CONFIG = {`n" +
           "    ads: false,`n" +
           "    support: $s,`n" +
           "    supportProductId: `"$productId`",`n" +
           "    supportPriceLabel: `"$priceLabel`"`n" +
           "};`n"
}
...
function Build-Apk([hashtable]$cfg) {
    ...
    if ((Test-Path $androidDir) -and (Test-Path $stateFile)) {
        try {
            ...
            } elseif ([bool]$prev.PlayMode -ne [bool]$cfg.PlayMode) {
                Write-Log "== Google Play mode changed: regenerating the Android project"
                Remove-Item $androidDir -Recurse -Force
            } elseif ($cfg.PlayMode -and (($(if ($prev.PlayVariant) { [string]$prev.PlayVariant } else { "ads" })) -ne [string]$cfg.PlayVariant)) {
                # ads <-> support swaps the plugin set (AdMob in or out), and
                # Capacitor bakes every installed plugin into the native
                # project - so both node_modules and android must go.
                Write-Log "== Play variant changed ($($prev.PlayVariant) -> $($cfg.PlayVariant)): regenerating the project"
                Remove-Item $androidDir -Recurse -Force
                foreach ($stale in @("node_modules", "package.json", "package-lock.json")) {
                    $sp = Join-Path $projDir $stale
    ...
    if ($cfg.PlayMode) {
        if (-not $cfg.PlayVariant) { $cfg.PlayVariant = "ads" }
        if ($cfg.PlayVariant -eq "ads") {
            if ($cfg.AdMobAppId -notmatch '^ca-app-pub-\d+~\d+$') {
                throw "Google Play mode needs your AdMob App ID (looks like ca-app-pub-1234567890123456~1234567890). Get it from the AdMob console under Apps."
            }
            if ($cfg.AdUnitId -notmatch '^ca-app-pub-\d+/\d+$') {
                throw "Google Play mode needs an interstitial ad unit ID (looks like ca-app-pub-1234567890123456/1234567890). Create one in the AdMob console under Ad units."
            }
            if (-not $cfg.ProductId) { $cfg.ProductId = "remove_ads" }
        } else {
            # Support variant: no ads, so no AdMob IDs needed at all.
            if (-not $cfg.ProductId) { $cfg.ProductId = "support_dev" }
        }
        if (-not $cfg.PriceLabel) { $cfg.PriceLabel = '$0.99' }
    }
    ...
    if ($cfg.PlayMode) {
        if ($cfg.PlayVariant -eq "support") {
            Write-Log "Mode:    Google Play (support-the-developer button, no ads) + itch"
        } else {
            Write-Log "Mode:    Google Play (ads + remove-ads purchase) + itch"
        }
    }
    ...
    if (-not (Test-Path (Join-Path $projDir "package.json"))) {
        ...
        $playDeps = ""
        if ($cfg.PlayMode) {
            # Support variant ships billing only - keeping AdMob out keeps the
            # AD_ID/ADSERVICES permissions out of the Play manifest entirely.
            if ($cfg.PlayVariant -eq "support") {
                $playDeps = '    "@capgo/native-purchases": "^8.0.0",' + "`n"
            } else {
                $playDeps = '    "@capacitor-community/admob": "^8.0.0",' + "`n" +
                            '    "@capgo/native-purchases": "^8.0.0",' + "`n"
            }
        }
        Write-TextFile (Join-Path $projDir "package.json") (@"
{
  ...
  "private": true,
  "dependencies": {
__PLAY_DEPS__    "@capacitor/android": "^8.0.0",
    "@capacitor/app": "^8.0.0",
    "@capacitor/cli": "^8.0.0",
    "@capacitor/core": "^8.0.0"
  }
}
"@).Replace("__PLAY_DEPS__", $playDeps)
    }
...
"@
    ...
    if ($cfg.PlayMode) {
        if ($cfg.PlayVariant -eq "support") {
            if (-not (Test-Path (Join-Path $projDir "node_modules\@capgo\native-purchases"))) {
                Run-Cmd "Adding Google Play billing plugin" $projDir "npm.cmd" @("install", "--no-fund", "--no-audit", "@capgo/native-purchases@8")
            }
        } elseif (-not (Test-Path (Join-Path $projDir "node_modules\@capacitor-community\admob"))) {
            Run-Cmd "Adding Google Play plugins (AdMob + billing)" $projDir "npm.cmd" @("install", "--no-fund", "--no-audit", "@capacitor-community/admob@8", "@capgo/native-purchases@8")
        }
    }
...
"@
    ...
    if ($cfg.PlayMode) {
        $ih = Edit-Once $ih '</body>' ("    <script type=`"text/javascript`" src=`"js/mobile-config.js`"></script>`n    <script type=`"text/javascript`" src=`"js/monetization.js`"></script>`n    </body>")
    }
    ...
    # --- Google Play mode: monetization script + per-store config -----------
    # www/ carries the PRODUCTION config (real IDs, real ads); build-time
    # asset overlays swap it per variant: playDebug -> Google test ads,
    # itch -> everything off. The itch APK stays clean.
    if ($cfg.PlayMode) {
        if ($cfg.PlayVariant -eq "support") {
            Write-TextFile (Join-Path $wwwDir "js\monetization.js") (Get-Content (Join-Path $TemplateDir "support.js.template") -Raw)
            Write-TextFile (Join-Path $wwwDir "js\mobile-config.js") (New-SupportConfig $true $cfg.ProductId $cfg.PriceLabel)
        } else {
            Write-TextFile (Join-Path $wwwDir "js\monetization.js") (Get-Content (Join-Path $TemplateDir "monetization.js.template") -Raw)
            Write-TextFile (Join-Path $wwwDir "js\mobile-config.js") (New-MobileConfig $true $false $cfg.AdUnitId $cfg.ProductId $cfg.PriceLabel)
        }
    }
    ...
    if ($cfg.PlayMode -and $cfg.PlayVariant -ne "support") {
        if ($mc -notmatch "com\.google\.android\.gms\.ads\.APPLICATION_ID") {
            $adsMeta = "        <meta-data`n" +
                       "            android:name=`"com.google.android.gms.ads.APPLICATION_ID`"`n" +
                       "            android:value=`"$($cfg.AdMobAppId)`" />`n" +
                       "        <meta-data`n" +
                       "            android:name=`"com.google.android.gms.ads.DELAY_APP_MEASUREMENT_INIT`"`n" +
                       "            android:value=`"true`" />`n" +
                       "    </application>"
            $mc = Edit-Once $mc '</application>' $adsMeta
        } else {
            # keep the App ID current if the user edited it
            $mc = [regex]::Replace($mc, '(com\.google\.android\.gms\.ads\.APPLICATION_ID"[^>]*?android:value=")[^"]*', ('${1}' + $cfg.AdMobAppId))
        }
    }
...
"@)
    ...
    if ($cfg.PlayMode -and $gc -notmatch "flavorDimensions") {
        $gc = $gc.Replace("    signingConfigs {", @"
    // play = Google Play build (ads + billing); itch = clean sideload build.
    flavorDimensions = ["store"]
    productFlavors {
        play { dimension "store" }
        itch {
            dimension "store"
            // installable alongside the Play version
            applicationIdSuffix ".itch"
        }
    }
    signingConfigs {
"@)
    ...
    # --- Google Play mode: per-variant overlays ------------------------------
    if ($cfg.PlayMode) {
        # itch: strip ad/billing permissions and the AdMob auto-init provider.
        $itchDir = Join-Path $androidDir "app\src\itch"
        New-Item -ItemType Directory -Force (Join-Path $itchDir "assets\public\js") | Out-Null
        Write-TextFile (Join-Path $itchDir "AndroidManifest.xml") @"
<?xml version="1.0" encoding="utf-8"?>
<!-- itch flavor: strip ad/billing permissions and the AdMob auto-init
     provider that the SDKs merge in. The SDK classes still ship (unused,
     never initialized) but the app requests nothing and contacts nothing. -->
...
"@
        if ($cfg.PlayVariant -eq "support") {
            Write-TextFile (Join-Path $itchDir "assets\public\js\mobile-config.js") (New-SupportConfig $false "" "")
        } else {
            Write-TextFile (Join-Path $itchDir "assets\public\js\mobile-config.js") (New-MobileConfig $false $false "" "" "")
        }

        # playDebug: ads variant swaps in Google's public TEST interstitial so
        # the developer can test on their own phone without touching real ads
        # (account-ban risk). Support variant has no ads - same config as
        # release, just for checking the button (billing itself only works
        # once the app is on Play with license testers).
        $pdDir = Join-Path $androidDir "app\src\playDebug\assets\public\js"
        New-Item -ItemType Directory -Force $pdDir | Out-Null
        if ($cfg.PlayVariant -eq "support") {
            Write-TextFile (Join-Path $pdDir "mobile-config.js") (New-SupportConfig $true $cfg.ProductId $cfg.PriceLabel)
        } else {
            Write-TextFile (Join-Path $pdDir "mobile-config.js") (New-MobileConfig $true $true "ca-app-pub-3940256099942544/1033173712" $cfg.ProductId $cfg.PriceLabel)
        }
    }
    ...
    if ($cfg.PlayMode) {
        Run-Cmd "Building Play AAB + test APK + itch APK (first build downloads Gradle deps)" $androidDir ".\gradlew.bat" @("bundlePlayRelease", "assemblePlayDebug", "assembleItchRelease", "--no-daemon", "--console=plain")

        $aab     = Join-Path $androidDir "app\build\outputs\bundle\playRelease\app-play-release.aab"
        $apkTest = Join-Path $androidDir "app\build\outputs\apk\play\debug\app-play-debug.apk"
        $apk     = Join-Path $androidDir "app\build\outputs\apk\itch\release\app-itch-release.apk"
    ...
    Write-Log ""
    Write-Log "=========================================================="
    if ($cfg.PlayMode -and $cfg.PlayVariant -eq "support") {
        Write-Log " DONE - three outputs, versionCode $vc, version $($cfg.VersionName):"
        ...
        Write-Log " DONE - three outputs, versionCode $vc, version $($cfg.VersionName):"
        Write-Log "   Play upload (AAB):     $destAab"
        Write-Log "   Phone testing (TEST ads, safe to tap): $destTest"
        Write-Log "   itch.io (no ads, no billing):          $dest"
        Write-Log " Never tap real ads on your own device - test with the"
        Write-Log " TEST-ads APK. The AAB uses your real AdMob IDs."
    } else {
        Write-Log " DONE: $dest"
...
if ($NoGui) {
    ...
    $cfg = @{
        ...
        PlayMode = [bool]$PlayMode; PlayVariant = $PlayVariant; AdMobAppId = $AdMobAppId
        AdUnitId = $AdUnitId; ProductId = $ProductId; PriceLabel = $PriceLabel
    }
...
# ---- build button ----
# ---- Google Play row ----
$chkPlay = New-Control "CheckBox" 18 496 340 22 "Google Play build (ads or support button)" $form
$btnPlayIds = New-Control "Button" 366 494 110 26 "Play IDs..." $form
$lblPlayState = New-Control "Label" 484 499 140 18 "" $form
$lblPlayState.Font = New-Object System.Drawing.Font("Segoe UI", 7.5)
...
function Update-PlayStateLabel {
    if (-not $chkPlay.Checked) { $lblPlayState.Text = ""; return }
    if ($script:PlayIds.Variant -eq "support") {
        $lblPlayState.Text = "Support button (no ads)"
    } elseif ($script:PlayIds.AdMobAppId -match '^ca-app-pub-\d+~\d+$' -and
        $script:PlayIds.AdUnitId -match '^ca-app-pub-\d+/\d+$') {
        $lblPlayState.Text = "Ads - IDs set"
    } else {
        $lblPlayState.Text = "IDs missing - click Play IDs..."
    }
}

$btnBuild = New-Control "Button" 16 526 600 38 "BUILD  APK" $form
...
$lblLog = New-Control "Label" 16 572 100 18 "Build log" $form
$btnClear = New-Control "Button" 1132 566 84 26 "Clear log" $form
$script:LogBox = New-Control "TextBox" 16 596 1200 170 "" $form
    ...
    if ($d.ShowDialog() -eq "OK") {
        ...
        if (Test-Path $state) {
            try {
                ...
                if ($null -ne $s.PlayMode) { $chkPlay.Checked = [bool]$s.PlayMode }
                if ($s.PlayVariant) { $script:PlayIds.Variant = [string]$s.PlayVariant }
                if ($s.AdMobAppId) { $script:PlayIds.AdMobAppId = [string]$s.AdMobAppId }
                if ($s.AdUnitId)   { $script:PlayIds.AdUnitId   = [string]$s.AdUnitId }
                if ($s.ProductId)  { $script:PlayIds.ProductId  = [string]$s.ProductId }
                if ($s.PriceLabel) { $script:PlayIds.PriceLabel = [string]$s.PriceLabel }
                Update-PlayStateLabel
...
function Show-PlayIdsDialog {
    $dark = $chkDark.Checked
    $f = New-Object System.Windows.Forms.Form
    $f.Text = "Google Play IDs (per game)"
    $f.ClientSize = New-Object System.Drawing.Size(620, 384)
    ...
    $rows = @(
        @("AdMob App ID",          "AdMobAppId", "From the AdMob console under Apps. Looks like ca-app-pub-1234...~1234..."),
        @("Interstitial unit ID",  "AdUnitId",   "An Interstitial ad unit for this game. Looks like ca-app-pub-1234.../1234..."),
        @("Remove-ads product ID", "ProductId",  "The in-app product in Play Console (default: remove_ads)"),
        @("Price label",           "PriceLabel", "Fallback shown before Play returns the real localized price")
    )
    $boxes = @{}
    $y = 16

    # monetization variant: ads + remove-ads purchase, or just a tip button
    $lblVar = New-Object System.Windows.Forms.Label
    $lblVar.Location = New-Object System.Drawing.Point(16, ($y + 3))
    $lblVar.Size = New-Object System.Drawing.Size(150, 18); $lblVar.Text = "Monetization"
    $f.Controls.Add($lblVar)
    $radAds = New-Object System.Windows.Forms.RadioButton
    $radAds.Location = New-Object System.Drawing.Point(170, $y)
    $radAds.Size = New-Object System.Drawing.Size(220, 22)
    $radAds.Text = "Ads + remove-ads purchase"
    $radSup = New-Object System.Windows.Forms.RadioButton
    $radSup.Location = New-Object System.Drawing.Point(392, $y)
    $radSup.Size = New-Object System.Drawing.Size(212, 22)
    $radSup.Text = "Support button only (no ads)"
    if ($script:PlayIds.Variant -eq "support") { $radSup.Checked = $true } else { $radAds.Checked = $true }
    $f.Controls.Add($radAds); $f.Controls.Add($radSup)
    $y += 32

    foreach ($r in $rows) {
        $l = New-Object System.Windows.Forms.Label
        $l.Location = New-Object System.Drawing.Point(16, ($y + 3))
        $l.Size = New-Object System.Drawing.Size(150, 18); $l.Text = $r[0]
        $f.Controls.Add($l)
        $t = New-Object System.Windows.Forms.TextBox
        $t.Location = New-Object System.Drawing.Point(170, $y)
        $t.Size = New-Object System.Drawing.Size(430, 22)
        $t.Text = [string]$script:PlayIds[$r[1]]
        $f.Controls.Add($t); $boxes[$r[1]] = $t
        $h = New-Object System.Windows.Forms.Label
        $h.Location = New-Object System.Drawing.Point(170, ($y + 24))
        $h.Size = New-Object System.Drawing.Size(430, 16); $h.Text = $r[2]
        $h.Font = New-Object System.Drawing.Font("Segoe UI", 7.5)
        $f.Controls.Add($h)
        $y += 52
    }
    $note = New-Object System.Windows.Forms.Label
    $note.Location = New-Object System.Drawing.Point(16, $y)
    $note.Size = New-Object System.Drawing.Size(584, 48)
    ...
    $note.Font = New-Object System.Drawing.Font("Segoe UI", 7.5)
    $f.Controls.Add($note)

    # grey out the AdMob rows when the tip-button variant is picked
    $syncVariant = {
        $sup = $radSup.Checked
        $boxes["AdMobAppId"].Enabled = -not $sup
        $boxes["AdUnitId"].Enabled   = -not $sup
        if ($sup) {
            if (-not $boxes["ProductId"].Text -or $boxes["ProductId"].Text -eq "remove_ads") {
    ...
    $radAds.Add_CheckedChanged($syncVariant)
    $radSup.Add_CheckedChanged($syncVariant)
    & $syncVariant

    $ok = New-Object System.Windows.Forms.Button
    $ok.Text = "OK"; $ok.Size = New-Object System.Drawing.Size(100, 30)
    $ok.Location = New-Object System.Drawing.Point(390, 340)
    $ok.DialogResult = "OK"
    ...
    if ($dark) {
        $f.BackColor = [System.Drawing.Color]::FromArgb(30, 30, 32)
        foreach ($c in $f.Controls) {
            if ($c -is [System.Windows.Forms.TextBox]) {
                $c.BackColor = [System.Drawing.Color]::FromArgb(45, 45, 48)
                $c.ForeColor = [System.Drawing.Color]::FromArgb(238, 238, 240)
            } elseif ($c -is [System.Windows.Forms.Button]) {
                $c.BackColor = [System.Drawing.Color]::FromArgb(62, 62, 66)
                $c.ForeColor = [System.Drawing.Color]::White
                $c.FlatStyle = "Flat"
            } else {
                $c.ForeColor = [System.Drawing.Color]::FromArgb(228, 228, 230)
            }
        }
    }

    if ($f.ShowDialog($form) -eq "OK") {
        $script:PlayIds.Variant    = $(if ($radSup.Checked) { "support" } else { "ads" })
        $script:PlayIds.AdMobAppId = $boxes["AdMobAppId"].Text.Trim()
        $script:PlayIds.AdUnitId   = $boxes["AdUnitId"].Text.Trim()
        $script:PlayIds.ProductId  = $boxes["ProductId"].Text.Trim()
        $script:PlayIds.PriceLabel = $boxes["PriceLabel"].Text.Trim()
    }
    $f.Dispose()
    Update-PlayStateLabel
}

$btnPlayIds.Add_Click({ Show-PlayIdsDialog })
$chkPlay.Add_CheckedChanged({ Update-PlayStateLabel })
...
$btnBuild.Add_Click({
    ...
    if ($chkPlay.Checked) {
        if ($script:PlayIds.Variant -eq "support") {
            if (-not $script:PlayIds.ProductId) { $script:PlayIds.ProductId = "support_dev" }
        } else {
            if ($script:PlayIds.AdMobAppId -notmatch '^ca-app-pub-\d+~\d+$') {
                $problems += "- Google Play build needs your AdMob App ID (Play IDs... button). It looks like ca-app-pub-1234567890123456~1234567890."
            }
            if ($script:PlayIds.AdUnitId -notmatch '^ca-app-pub-\d+/\d+$') {
                $problems += "- Google Play build needs an interstitial ad unit ID (Play IDs... button). It looks like ca-app-pub-1234567890123456/1234567890."
            }
            if (-not $script:PlayIds.ProductId) { $script:PlayIds.ProductId = "remove_ads" }
        }
        if (-not $script:PlayIds.PriceLabel) { $script:PlayIds.PriceLabel = '$0.99' }
    }
    ...
    try {
        ...
        $cfg = @{
            ...
            PlayMode = $chkPlay.Checked
            PlayVariant = $(if ($script:PlayIds.Variant) { $script:PlayIds.Variant } else { "ads" })
            AdMobAppId = $script:PlayIds.AdMobAppId; AdUnitId = $script:PlayIds.AdUnitId
            ProductId = $script:PlayIds.ProductId; PriceLabel = $script:PlayIds.PriceLabel
        }
        $apk = Build-Apk $cfg
