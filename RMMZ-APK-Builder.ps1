...
function Build-Apk([hashtable]$cfg) {
    ...
    if ($cfg.PlayMode) { Write-Log "Mode:    Google Play (ads + remove-ads purchase) + itch" }
...
"@
    ...
    if ($cfg.PlayMode -and -not (Test-Path (Join-Path $projDir "node_modules\@capacitor-community\admob"))) {
...
"@
        ...
        # playDebug: Google's public TEST interstitial, so the developer can
        # test on their own phone without touching real ads (account-ban risk).
    ...
    if ($cfg.PlayMode) {
        ...
        $destTest = Join-Path $cfg.OutputDir ($safeTitle + " v" + $cfg.VersionName + " (Play test ads).apk")
...
if ($NoGui) {
    ...
    $cfg = @{
        ...
        PlayMode = [bool]$PlayMode; AdMobAppId = $AdMobAppId
...
# ---- build button ----
# ---- Google Play row ----
$chkPlay = New-Control "CheckBox" 18 496 340 22 "Google Play build (ads + remove-ads purchase)" $form
...
function Update-PlayStateLabel {
    ...
    if ($script:PlayIds.AdMobAppId -match '^ca-app-pub-\d+~\d+$' -and
        $script:PlayIds.AdUnitId -match '^ca-app-pub-\d+/\d+$') {
        $lblPlayState.Text = "IDs set"
    } else {
...
function Show-PlayIdsDialog {
    ...
    $f.ClientSize = New-Object System.Drawing.Size(620, 320)
    ...
    $note.Size = New-Object System.Drawing.Size(584, 34)
    ...
    $ok.Location = New-Object System.Drawing.Point(390, 276)
    $ok.DialogResult = "OK"
    $cancel = New-Object System.Windows.Forms.Button
    $cancel.Text = "Cancel"; $cancel.Size = New-Object System.Drawing.Size(100, 30)
    $cancel.Location = New-Object System.Drawing.Point(500, 276)
